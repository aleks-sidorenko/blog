...
**Connecting your bank is the mechanism that makes it automatic.** That's the entire reason banks are in this story. You connect an account once, and from then on every card payment, transfer and fee lands in the ledger by itself, already categorized, whether or not you ever open the app. There's no import ritual and no monthly reconciliation session — the automatic import *is* the product, and everything else in this section exists to cover what it can't reach.
